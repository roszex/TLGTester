...
function preloadImages() {
    const images = ['../22_page_photo.jpeg'];
    images.forEach(src => {
        const img = new Image();
        img.src = src;
    });
...
window.addEventListener('load', function() {
    ...
    if (window.progressManager) {
        window.progressManager.savePage(22);
    }
});
...
function goToNextPage() {
    ...
    setTimeout(() => {
        ...
        if (window.progressManager) {
            ...
            // Переходим на 23-ю страницу
            const newUrl = baseUrl + '/../page_23/index.html' + (userId ? `?user_id=${userId}` : '');
