...
function preloadImages() {
    const images = ['../24_page_photo_1.jpeg', '../24_page_photo_2.jpeg'];
    images.forEach(src => {
        const img = new Image();
        img.src = src;
    });
...
window.addEventListener('load', function() {
    ...
    if (window.progressManager) {
        window.progressManager.savePage(24);
    }
});
...
function goToNextPage() {
    ...
    setTimeout(() => {
        ...
        if (window.progressManager) {
            ...
            // Переходим на 25-ю страницу
            const newUrl = baseUrl + '/../page_25/index.html' + (userId ? `?user_id=${userId}` : '');
