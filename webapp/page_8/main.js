...
function preloadImages() {
    const images = ['../9_page_photo.jpeg'];
    images.forEach(src => {
        const img = new Image();
        img.src = src;
    });
...
window.addEventListener('load', function() {
    ...
    if (window.progressManager) {
        window.progressManager.savePage(8);
    }
});

// Функция показа "Ого!" эффекта
function showWow() {
    ...
    setTimeout(() => {
        ...
        if (window.progressManager) {
            ...
            const newUrl = baseUrl + '/../page_9/index.html' + (userId ? `?user_id=${userId}` : '');
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (nextBtn) {
        nextBtn.addEventListener('click', showWow);
    }
});
